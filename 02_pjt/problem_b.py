...
def vote_average_movies():
    ...
    # print(results)
    # 새 리스트를 설정하고 results의 길이만큼 results를 돌면서 'vote_average'의 값을 뽑아내는데 
    vote_8 = []
    for i in range(len(results)):
      if results[i]['vote_average'] >= 8: # 8이상이면 새 리스트 속에 집어 넣는다(영화목록). 그리고 그 리스트를 반환한다.
        vote_8.append(results[i])
    return vote_8

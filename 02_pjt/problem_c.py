...
def ranking():
    ...
    # b번에서 뽑았던 것처럼 영화목록 리스트를 뽑아내고
    # 람다함수와 sort함수를 이용해 'vote_average'의 값이 큰순으로 result를 정렬시킨다.
    results.sort(key=lambda x: x['vote_average'], reverse=True)
    final = list(results[:5])     # result의 5번쨰까지 뽑아내서 인기있는 영화목록을 새로 final리스트에 집어 넣는다
    return final
